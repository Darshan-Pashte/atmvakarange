...
export default function AtmOutOfServiceModal({ atmId, open, handleClose }) {
  ...
  const getData = async () => {
    ...
    try {
      ...
      const response = await postApiData(apiList.ATM_SERVICE_DATA + `inservice`, payload);
      ...
      >
        <Box sx={style} className={classes.popup}>
          <div className={classes.header}>
            <div className={classes.headerTitle} style={{ fontSize: '18px', padding: '8px' }}>ATM ID - {atmId}</div>
