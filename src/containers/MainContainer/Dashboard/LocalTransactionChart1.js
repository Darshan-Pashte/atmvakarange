...
const ApexChart = ({graph}) => {
  const hourLabels = graph?.map(entry => `${entry.issuhourStr}hrs`);
    const countValues = graph?.map(entry => entry.issucnt);
  const chartData = {
    series: [
      {
        name: 'Count',
    ...
    options: {
      ...
      fill: {
        opacity: 1,
        colors: ['#AD5389'], // Change this to the desired color code
      },
      tooltip: {
        y: {
...
// const LocalTransactionChart1 = ({graph}) => {
//   const hourLabels = graph?.map(entry => `${entry.issuhourStr}hrs`);
//   const countValues = graph?.map(entry => entry.issucnt);
//   const chartData = {
//     series: [
...
// export default LocalTransactionChart1;
