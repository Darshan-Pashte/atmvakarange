...
console.log("state", state);
  ...
  useEffect(() => {
    ...
    setValue("servicecode", state?.servicecode);
    setValue("pvk", state?.pvk);
    setValue("issposport", state?.issposport);
    setValue("issposip", state?.issposip);
    setValue("issatmport",state?.issatmport);
    setValue("issatmip",state?.issatmip);
    
    setValue("hostport", state?.hostport);
    
    
    
    setValue("hostip", state?.hostip);
    setValue("dectable", state?.dectable);
    setValue("binname", state?.binname);
    setValue("bin", state?.bin);
    setValue("bankcode", state?.bankcode);
    setValue("bankcd", state?.bankcd);
    setValue("acqid", state?.acqid );
    setValue("mobilebin",compareIdAndReturnObject(MobileBIN,state?.mobilebinyn));
    setValue("actformat", compareIdAndReturnObject(ACTFormat,state?.actFormat));
  ...
  const onSubmits = async (data) => {
    try {
      ...
      const payload = {
        ...
        issposport: data.issatmport,
        mobilebinyn:data.mobilebin.code ,
        actFormat: data.actformat.code
    
      };
      console.log('payload',payload)
      const response = await postApiData(apiList.LOCAL_BIN_MASTER_MODIFY, payload);
      if (response?.status == true) {
      
        SweetAlertPopup(response.message, "Success", "success");
        // reset()
       navigate('/localbinmaster')
        setIsloading(false);
      } else {
    ...
    <>
      ...
      >
        ...
        <div className={classes.Sbox}>
          <div className={classes.bluerow}>Bin Master Modify</div>
          <div>
...
export default LocalBinMasterModify;
