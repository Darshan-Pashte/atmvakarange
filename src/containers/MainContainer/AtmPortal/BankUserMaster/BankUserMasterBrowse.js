...
import ViewModalDelete from "../../../../components/common/ViewModalDelete";
...
const BankUserMasterBrowse = () => {
  ...
  const url = "/bankusermaster/modify";
  const url1 = "/bankusermst";
...
const openModal = (rowData) => {
  ...
  const headers = columns?.filter((column) => column?.label !== "View" && column?.label !== "Modify" && column?.label !== "Delete").map((column) => column?.label);
  ...
  },
  {
    ...
    options: {
        ...
        customBodyRender: (value, { rowIndex }) => {
            return (
                <>
                    {/* <EditsIcons rowIndex={rowIndex} data={atmMasterList}  handleNavigate={handleNavigate} url={url} topLevelPath={topLevelPath} url1={url1} /> */
                     <DeleteIcon sx={{ color: "black" ,cursor:'pointer'}} onClick={()=>SwalCall(atmMasterList[rowIndex])}/> 
                     }
                </>
            );
        },
    ...
    },
    {
      ...
      options: {
        ...
        //   value === "0" ? "InActive" : value === "1" ? "Active" : value === '3' ? 'Block' :
        //   value === '7' ? ' New User' : value
        // )
...
console.log('data',data)
    ...
    try {
      ...
      const response = await postApiData(
        apiList.BANK_USER_MASTER_BROWSE + `?pageNo=${currentPage}&pageSize=${goPageNumber}`,
        payload
      );
  ...
  const handleCreateAccount = async (rowData , para) => {
    try {
      ...
      const response = await postApiData(apiList.BANK_USER_MASTER_DELETE, payload);
      ...
      >
        <div className={classes.Sbox}>
          <div className={classes.bluerow}>
            ...
            <div>
              <ColorButton1
                onClick={() => navigate("/bankusermaster/create")}
                variant="contained"
              >
                Create
              </ColorButton1>
...
export default BankUserMasterBrowse;
