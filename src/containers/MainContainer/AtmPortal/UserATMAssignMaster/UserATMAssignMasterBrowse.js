...
const UserATMAssignMasterBrowse = () => {
  const {
    control,
    handleSubmit,
    setValue,
  ...
  const columns = [
    ...
    },
    {
      ...
      options: {
        ...
        //   value === "A" ? "Active" : value === "D" ? "De-Active" : value
        // )
      },
    },
    {
  ...
  const getTransactionListView = async (currentPage,data = payloadData) => {
    ...
    try {
      ...
      const response = await postApiData(
        apiList.USER_ATM_ASSIGN_BROWSE + `?pageNo=${currentPage}&pageSize=${goPageNumber}`,
        payload
      );
...
export default UserATMAssignMasterBrowse;
